...
#include <random>      // For sort, reverse
 ...
 * test failures.
 */
#define checkCondition(expr, reason) \
    docheckCondition((expr), (reason), #expr, __LINE__)
 ...
 */
void docheckCondition(bool expr,
                      string reason,
                      string expression,
                      int lineNumber) {
    if (expr) {
        cout << GREEN << "  PASS: " << NONE << reason << endl;
    } else {
        cout << RED << "! FAIL: " << NONE << reason << endl;
        cout << "     Test failed on line " << lineNumber
             << " of pqueue-test.cpp" << endl;
...
void basicStructuralTests() {
    ...
    try {
        ...
        {
            logInfo(
                "These tests will check size() isEmpty() without calling "
                "dequeueMin().");

            PQueue queue;
            checkCondition(queue.isEmpty(),
                           "New priority queue should be empty.");
            checkCondition(queue.size() == 0,
                           "New priority queue should have size 0.");
            ...
            for (int i = 0; i < 5; i++) {
                ...
                checkCondition(
                    queue.size() == i + 1,
                    "Queue should have proper size after inserting a value.");
                checkCondition(
                    !queue.isEmpty(),
                    "Queue containing elements should not be empty.");
            }
        }

        /* Slightly more complex test: Enqueue and dequeue elements, ensuring
         * the size matches at each step.
         */
        {
            ...
            for (int i = 5; i > 0; i--) {
                checkCondition(queue.size() == i,
                               "Queue should have proper size after dequeues.");
                checkCondition(!queue.isEmpty(),
                               "Queue should not be empty before all elements "
                               "are removed.");
                queue.dequeueMin();
            }
            ...
            checkCondition(
                queue.size() == 0,
                "After removing all elements, the queue should have size 0.");
            checkCondition(
                queue.isEmpty(),
                "After removing all elements, the queue should be empty.");
        }

        /* Getting harder: The value dequeued should always match the value of
         * peek(). */
        {
            logInfo(
                "This next test will check whether peek() matches "
                "dequeueMin().");
            PQueue queue;
            ...
            while (!queue.isEmpty()) {
                ...
                checkCondition(queue.dequeueMin() == expected,
                               "Value returned by peek() matches value "
                               "returned by dequeueMin()");
            }
        }

        /* A different one - let's make sure that peeking at an empty queue
         * causes an error.
         */
        {
            PQueue queue;
            ...
            try {
                logInfo(
                    "About to peek into an empty queue.  This may cause a "
                    "crash");
            ...
            checkCondition(didThrow,
                           "Priority queue uses 'error' when peek() called on "
                           "empty queue.");
        }
        ...
        /* In the same vein - what happens if we dequeue from an empty queue? */
        {
            ...
            try {
                logInfo(
                    "About to dequeue from an empty queue.  This may cause a "
                    "crash");
                logInfo("if your implementation is incorrect.");
                queue.dequeueMin();
            } catch (std::runtime_error&) {
            ...
            checkCondition(didThrow,
                           "Priority queue uses 'error' when dequeueMin() "
        ...
        cout << RED << "! FAIL: " << NONE << "Unknown exception." << endl;
    }
...
void sortAscendingTests() {
    ...
    try {
        ...
        /* Basic test: Feed the strings A through H into the priority queue and
         * confirm that they come back in the same order.
         */
        {
            logInfo(
                "Enqueuing A - H into the queue and confirming it comes back "
                "sorted.");
            PQueue queue;
            ...
            for (char ch = 'A'; ch <= 'H'; ch++) {
                ...
                checkCondition(queue.dequeueMin() == expected,
                               "Queue should yield " + expected + ".");
            }
        }

        /* Harder test: Sort 10 random strings and confirm that the priority
         * queue hands them back in the same order.
         */
        {
            logInfo(
                "Enqueuing ten sorted random strings and confirming it comes "
                "back sorted.");
            vector<string> randomValues;
            for (int i = 0; i < 10; i++) {
                randomValues.push_back(randomString());
            }
            ...
            for (int i = 0; i < randomValues.size(); i++) {
                checkCondition(queue.dequeueMin() == randomValues[i],
                               "Expecting to get value " + randomValues[i] +
                                   " from queue.");
            }
        }

        /* Much harder test: Sort 10000 random strings and confirm that the
         * priority queue hands them back in the same order.
         */
        {
            ...
            checkCondition(isCorrect,
                           "Queue correctly sorted 10000 random strings.");
        }
...
void sortDescendingTests() {
    ...
    try {
        ...
        /* Basic test: Feed the strings H through A into the priority queue and
         * confirm that they come back in the same order.
         */
        {
            logInfo(
                "Enqueuing A - H in reverse order and checking that it comes "
                "back sorted.");
            PQueue queue;
        ...
        {
            logInfo(
                "Enqueuing ten reverse sorted random strings and confirming it "
                "comes back sorted.");
            vector<string> randomValues;
            for (int i = 0; i < 10; i++) {
                randomValues.push_back(randomString());
...
void sortRandomTests() {
    ...
    try {
        ...
        /* Basic test: Feed the strings H through A into the priority queue in
         * some order, then confirm that they come back in sorted order
         */
        {
            logInfo(
                "Enqueuing a random permutation of A - H and checking whether "
                "it leaves sorted.");
            ...
            for (char ch = 'A'; ch <= 'H'; ch++) {
                 ...
                 * converts the character ch into a one-character string.
                 */
                letters.push_back(string(1, ch));
            }

            /* Scramble the letters with the STL shuffle algorithm. */
            std::random_device rd;
            std::mt19937 grd(rd());
            shuffle(letters.begin(), letters.end(), grd);
        ...
        {
            logInfo(
                "Enqueuing 10 random strings and checking whether it leaves "
                "sorted.");
...
void sortCraftedTests() {
    ...
    try {
         ...
         */
        {
            ...
            sequence.push_back(string(1, 'A'));
            sequence.push_back(string(1, 'C'));
            sequence.push_back(string(1, 'E'));
            sequence.push_back(string(1, 'G'));
            sequence.push_back(string(1, 'B'));
            sequence.push_back(string(1, 'D'));
            sequence.push_back(string(1, 'F'));
            sequence.push_back(string(1, 'H'));
        ...
        {
            logInfo(
                "Sorting two sequences that need to be interleaved, version "
                "two.");
            vector<string> sequence;
            sequence.push_back(string(1, 'B'));
            sequence.push_back(string(1, 'D'));
        ...
        {
            logInfo(
                "Sorting two decreasing sequences that need to be "
                "interleaved.");
            vector<string> sequence;
            sequence.push_back(string(1, 'H'));
            sequence.push_back(string(1, 'F'));
        ...
        {
            logInfo(
                "Sorting two decreasing sequences that need to be interleaved, "
                "version 2.");
            vector<string> sequence;
            sequence.push_back(string(1, 'G'));
            sequence.push_back(string(1, 'E'));
...
void sortDuplicateTests() {
    ...
    try {
        ...
        /* Second test: Duplicate the letters A - D, in reverse order, and see
         * if they come back correctly.
         */
        {
        ...
        /* Fourth test: Enqueue the letters A - H, in reverse order, then
         * duplicate the sequence. */
        {
            logInfo(
                "Loading two reverse sorted sequences, one after the other.");
...
void printReplInstructions() {
    ...
    cout << "===============================" << endl;
    cout << "This environment allows you to type in commands that will be"
         << endl;
    cout << "executed on your priority queue.  The interpreter knows the"
         << endl;
    cout << "following commands:" << endl;
    cout << endl;
    cout << "   isEmpty:        Reports whether the priority queue is empty."
         << endl;
    cout << "   size:           Reports the size of the priority queue" << endl;
    cout << "   enqueue <str>:  Enqueues the string <str>" << endl;
    cout << "   peek:           Peeks at the minimum element of the priority "
            "queue."
         << endl;
    cout << "   dequeueMin:     Dequeues the minimum element of the priority "
            "queue."
         << endl;
    cout << "   quit:           Quits the interpret and returns to the menu."
         << endl;
    cout << endl;
    cout << "The first letter of any command can be used as a substitute"
         << endl;
...
void replTestPriorityQueue() {
    ...
    /* These curly braces introduce a new layer of scoping.  This ensures that
     * if your priority queue's destructor causes an error, the error occurs
                ...
                try {
                    if (action == "peek" || action == "p") {
                        ...
                        if (!getline(command, toEnqueue)) {
                            ...
                            cout << "Enqueued string \"" << toEnqueue << "\"."
                                 << endl;
                        }
                    } else {
 ...
 */
void displayMenu() {
    cout << "COSC 220 Priority Queue Testing Harness" << endl;
    cout << "=====================================" << endl;
    cout << REPL_VECTOR << ": Manually test VectorPriorityQueue" << endl;
    cout << TEST_VECTOR << ": Automatically test VectorPriorityQueue" << endl;
    cout << REPL_LINKED_LIST << ": Manually test LinkedListPriorityQueue"
         << endl;
    cout << TEST_LINKED_LIST << ": Automatically test LinkedListPriorityQueue"
         << endl;
    cout << REPL_DOUBLY_LINKED_LIST
         << ": Manually test DoublyLinkedListPriorityQueue" << endl;
    cout << TEST_DOUBLY_LINKED_LIST
         << ": Automatically test DoublyLinkedListPriorityQueue" << endl;
    cout << QUIT << ": Quit" << endl;
}
